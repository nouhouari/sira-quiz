...
7. [Regenerating Store Screenshots](#7-regenerating-store-screenshots)
...
## 7. Regenerating Store Screenshots

Store screenshots are device-framed, captioned PNGs generated by a headless
`flutter test` run — no emulator, no macOS, no store credentials required.
One run regenerates assets for **both** stores and **all** locales (en-US, fr-FR).

### Shared script (platform-independent)
...
# With FVM:
FLUTTER_BIN=~/fvm/versions/stable/bin/flutter ./tool/generate_store_screenshots.sh
```

### Fastlane lanes

Run either lane before uploading to refresh assets:
...
Both lanes invoke the shared script above — they are equivalent.

### CI workflow (`.github/workflows/screenshots.yml`)

- **Trigger:** `workflow_dispatch` (manual) or push to `main` when files under
  `lib/`, `test/store_screenshots_test.dart`, `lib/data/db/seed/questions_seed.json`,
  or `assets/fonts/` change.
- **Runner:** `ubuntu-latest` (fully headless — no display server needed).
- **Secrets required:** none.
- **Output:** PNGs uploaded as a `store-screenshots-<sha>` artifact (30-day
  retention). Download from the Actions run page and review before committing
  to the repo or uploading to the stores.

To trigger manually:
```
GitHub → Actions → "Regenerate Store Screenshots" → Run workflow
```
...
| Store | Locale | Path |
|-------|--------|------|
| Google Play | en-US | `android/fastlane/metadata/android/en-US/images/` |
| Google Play | fr-FR | `android/fastlane/metadata/android/fr-FR/images/` |
| App Store | en-US | `ios/fastlane/screenshots/en-US/` |
| App Store | fr-FR | `ios/fastlane/screenshots/fr-FR/` |

### Content review checklist

Before committing or uploading regenerated screenshots:

- [ ] All captions reviewed (see `_SCREENSHOT_COPY_DRAFT.md`)
- [ ] No tofu (all icon and font glyphs render correctly)
- [ ] Islamic content and (SWS) honorific validated
